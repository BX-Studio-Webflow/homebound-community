...
(() => {
  ...
  var HomeMapController = class _HomeMapController {
     ...
     */
    bindCardHover() {
      const cards = this.root.querySelectorAll(
        '[dev-target="feature-collection-item"][id]'
      );
      if (!cards.length) {
        console.error(
          'HomeMapController: No [dev-target="feature-collection-item"][id] cards found \u2014 add an id that matches a SVG highlight group id.'
        );
        return;
      }
      cards.forEach((card) => {
    ...
    highlight(highlightId) {
      ...
      for (const svgEl of this.svgEls) {
        svgEl.querySelector(`g#${CSS.escape(highlightId)}[data-attribute="feature"]`)?.classList.add("home-map__shape--active");
      }
      const escapedId = CSS.escape(highlightId);
      this.root.querySelector(`[dev-target="feature-collection-item"]#${escapedId}`)?.classList.add("home-map__card--active");
    }
    /**
    ...
    clearHighlight() {
      ...
      for (const svgEl of this.svgEls) {
        svgEl.querySelectorAll(".home-map__shape--active").forEach((el) => el.classList.remove("home-map__shape--active"));
      }
      this.root.querySelectorAll(".home-map__card--active").forEach((el) => el.classList.remove("home-map__card--active"));
    }
  };
