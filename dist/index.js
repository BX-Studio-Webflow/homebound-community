...
(() => {
  ...
  var GalleryController = class {
    ...
    imgObserver = null;
    init() {
      const galleries = document.querySelectorAll('[dev-target="image-gallery"]');
      if (!galleries.length) {
    ...
    observeGalleryImages() {
      const refresh = () => {
        this.cachedImgs = Array.from(
          document.querySelectorAll('img[dev-target="hidden-main-gallery-images"]')
        );
      };
      refresh();
      const container = document.querySelector(".hidden-main-gallery-collection");
      if (!container) {
